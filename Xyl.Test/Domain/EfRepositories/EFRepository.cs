...
using System.Web;
using Xyl.Test.Code.Expand.Linq;
using Xyl.Test.Code.Utilities;
using Xyl.Test.Domain.Repositories;
...
namespace Xyl.Test.Domain.EfRepositories
{
    ...
    public class EFRepository<TEntity> : IRepository<TEntity>, IUnitOfWorkRepository where TEntity : EntityBase
    {
        ...
        public IEnumerable<T> SqlQuery<T>(string sql, object[] ps)
        {
            return _dbContext.Database.SqlQuery<T>(sql, ps ?? new object[] { });
        }

        public int ExecuteSqlCommand(string sql, object[] ps)
        {
            return _dbContext.Database.ExecuteSqlCommand(sql, ps ?? new object[] { });
        }
    }
}
